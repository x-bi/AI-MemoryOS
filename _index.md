...
- `adapters/gate-source/`：Codex / Claude gate 与 bootstrap 的共享源和 adapter overlay。
- `adapters/codex/bootstrap.md`：Codex 每轮轻量加载入口，判断是否需要读取完整 gate；由 `tools/sync-adapter-gates.ps1` 生成。
- `adapters/codex/gate.md`：Codex 完整运行策略入口；由 `tools/sync-adapter-gates.ps1` 生成。
- `adapters/codex/external-config.md`：Codex OS 外部配置副本，用于换机或新软件快速恢复。
- `adapters/codex/skills/`：Codex 专用 skills 源目录。
- `adapters/codex/templates/`：Codex adapter gate/bootstrap 与 skill 生成模板。
- `adapters/claude/templates/`：Claude adapter gate/bootstrap 与 skill 生成模板。
...
- `adapters/claude/bootstrap.md`: Claude Code lightweight per-input loader, generated by `tools/sync-adapter-gates.ps1`. `C:\Users\btf\.claude\CLAUDE.md` is a bootstrap redirect that instructs Claude to read this file.
- `adapters/claude/CLAUDE.md`: Claude Code full Memory OS Gate, generated by `tools/sync-adapter-gates.ps1`. `bootstrap.md` decides when this full gate must be read.
