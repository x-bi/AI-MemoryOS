...
- 敏感内容边界以 `core/safety-rules.md` 为准，包括 token、密码、密钥、PII、客户数据、生产日志原文、未脱敏项目代码和商业敏感信息。
...
- future direction note 同样遵守 `core/safety-rules.md` 的敏感内容边界。

## 读取与写入边界

- L0-L3 定义和各 adapter 的 L1/L2 策略偏好以 adapter gate 文件为准。
- 读取 gate 只用于加载运行策略，不等于读取 Memory OS 正文。
- 如果 workflow / skill 本身需要读取 rules / domains / router 正文，应进入 L2。
